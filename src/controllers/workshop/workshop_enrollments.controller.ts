...
import * as WorkshopEnrollmentsModel from '../../models/workshop/workshop_enrollments.model';
import * as WorkshopModel from '../../models/workshop/workshop.model';
import { db } from '../../db';
...
export const getWorkshopEnrollments = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const enrollments = await WorkshopEnrollmentsModel.getWorkshopEnrollments(parseInt(workshop_id));
...
export const updateWorkshopEnrollmentStatus = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (!['enrolled', 'cancelled'].includes(status)) {
      return res.status(400).json({ error: 'Invalid status. Must be enrolled or cancelled' });
    }
