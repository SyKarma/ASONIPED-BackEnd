...
export const getRecords = async (
  ...
  console.log('🔍 Backend - Raw SQL results for first 3 records:');
  rows.slice(0, 3).forEach((row, index) => {
    console.log(`Record ${index}:`, {
      id: row.id,
      cpd_id: row.cpd_id,
      cpd_province: row.cpd_province,
      cpd_canton: row.cpd_canton,
      cpd_district: row.cpd_district,
      cpd_full_name: row.cpd_full_name
    });
  });
