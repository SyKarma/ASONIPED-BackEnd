...
import { LandingDonacionesCardModel } from '../models/landing-donaciones-card.model';

// Listar todas las cards
export const getAllLandingDonacionesCards = async (req: Request, res: Response): Promise<void> => {
  try {
...
// Obtener card por ID
export const getLandingDonacionesCardById = async (req: Request, res: Response): Promise<void> => {
  try {
...
export const createLandingDonacionesCard = async (req: Request, res: Response): Promise<void> => {
  ...
  if (!descripcion_card || typeof descripcion_card !== "string" || descripcion_card.length < 1 || descripcion_card.length > 250) {
    res.status(400).json({ error: "descripcion_card es requerida y debe tener entre 1 y 250 caracteres" });
    return;
  }
...
// Eliminar card
export const deleteLandingDonacionesCard = async (req: Request, res: Response): Promise<void> => {
  try {
    const id = parseInt(req.params.id);
