...
import { LandingVolunteerModel } from '../models/landing-volunteer.model';
...
export const getAllLandingVolunteers = async (req: Request, res: Response): Promise<void> => {
  try {
...
export const getLandingVolunteerById = async (req: Request, res: Response): Promise<void> => {
  try {
...
export const createLandingVolunteer = async (req: Request, res: Response): Promise<void> => {
  ...
  if (!descripcion || typeof descripcion !== "string" || descripcion.length > 500) {
    res.status(400).json({ error: "descripcion es requerida y máximo 500 caracteres" });
    return;
  }
  ...
  if (!subtitulo || typeof subtitulo !== "string" || subtitulo.length < 3 || subtitulo.length > 255) {
    res.status(400).json({ error: "subtitulo es requerido y debe tener entre 3 y 255 caracteres" });
    return;
  }
...
export const updateLandingVolunteer = async (req: Request, res: Response): Promise<void> => {
  const { titulo, descripcion, URL_imagen, subtitulo, texto_boton, color_boton } = req.body;
  ...
  if (!descripcion || typeof descripcion !== "string" || descripcion.length > 255) {
    res.status(400).json({ error: "descripcion es requerida y máximo 255 caracteres" });
    return;
  }
  if (!URL_imagen || typeof URL_imagen !== "string" || URL_imagen.length > 255 || !isValidUrl(URL_imagen)) {
    res.status(400).json({ error: "URL_imagen es requerida, válida y máximo 255 caracteres" });
...
export const deleteLandingVolunteer = async (req: Request, res: Response): Promise<void> => {
  try {
    const id = parseInt(req.params.id);
