...
router.post('/initialize', authenticateToken, async (req: any, res: any) => {
  try {
    ...
    res.status(500).json({
      success: false,
      error: error.message || 'Failed to initialize Google Drive service'
    });
  }
});
...
router.get('/auth/callback', async (req: any, res: any) => {
  try {
    ...
    console.log('🔍 Tokens received from Google:', {
      ...
      expiry_date: tokens.expiry_date ? tokens.expiry_date : 'missing',
      token_type: tokens.token_type || 'missing',
    ...
    console.log(`Found ${testResponse.data.files.length} files in Drive`);
